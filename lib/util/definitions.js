...
/**
 * The ref resolver collects the subschemas having an `$id` (eg. the ones
 * produced by TypeBox `Type.Recursive()`) as definitions, and rewrites the
 * references to them as `#/definitions/def-N`, but it leaves them inline.
 * Replaces each of them with a reference, so that the schema is emitted once,
 * as a top-level definition.
 * @param {object} schema - Resolved and cloned schema; mutated in place.
 * @param {object} ref - Ref resolver instance.
 * @param {Set<string>} [ignore] - Names of the definitions to leave as they are.
 * @returns {Record<string, object>} The `name -> schema` inline definitions found.
 */
function referenceInlineDefinitions (schema, ref, ignore = new Set()) {
  const names = new Map()
  const definitions = ref.definitions().definitions
  for (const name of Object.keys(definitions)) {
    const { $id } = definitions[name]
    if (typeof $id === 'string' && !ignore.has(name)) names.set($id, name)
  }
  ...
  walkSchema(schema, (subschema, pointer) => {
    ...
    // the ref resolver adds the consumed definitions to the root `definitions`:
    // those are moved to the top-level by `hoistDefinitions`
    if (pointer.length === 2 && DEFINITIONS_KEYWORDS.includes(pointer[0])) return
    // a fragment-only $id is an anchor, not a schema resource
    if (typeof subschema.$id !== 'string' || subschema.$id[0] === '#') return
    // the resolver ignores the duplicates of an `$id` (eg. the same schema
    // used by two routes): they are matched by `$id`, not by identity
    const name = names.get(subschema.$id)
    if (name === undefined) return
    found[name] = definitions[name]
    replacements.push([pointer.slice(), name])
  })

  // A nested match is inside a definition (replaced as a whole): skip it
  const replaced = []
  for (const [pointer, name] of replacements) {
    if (replaced.some((prefix) => prefix.every((token, i) => pointer[i] === token))) continue
    replaced.push(pointer)
    let parent = schema
    for (let i = 0; i < pointer.length - 1; i++) parent = parent[pointer[i]]
    parent[pointer[pointer.length - 1]] = { $ref: `#/definitions/${name}` }
  }
