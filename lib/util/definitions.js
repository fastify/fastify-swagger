...
/**
 * The ref resolver already knows the shared schemas: mapping them again as
 * external schemas of a single `resolve()` call evaluates their subschemas
 * against the wrong base URI, and it throws when one of those has a
 * fragment-only `$id` (`{ $id: '#address' }`). Only the schemas set by the
 * user in the plugin options must be provided.
 */
...
/**
 * Swagger and OpenAPI do not allow the `definitions` and `$defs` keywords into
 * a schema object. Every definition is moved out of the (already resolved and cloned)
 * schema and returned, so it can be added to the top-level definitions.
 * The `hoisted` map tracks `old pointer -> new name` to fix the references.
 */
