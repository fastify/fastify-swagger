...
function addHook (fastify, pluginOptions) {
  ...
  fastify.addHook('onRoute', (routeOptions) => {
    ...
    const shouldExposeHead = pluginOptions.exposeHeadRoutes === true || swaggerConfig.exposeHeadRoute === true

    if (!shouldExposeHead) {
      if (Array.isArray(routeOptions.method)) {
        const filteredMethods = routeOptions.method.filter(m => m !== 'HEAD')
        if (filteredMethods.length === 0) {
          return
        }
        routeOptions = Object.assign({}, routeOptions, { method: filteredMethods })
      } else if (routeOptions.method === 'HEAD') {
        return
      }
    }
