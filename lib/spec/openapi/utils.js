...
function prepareDefaultOptions (opts) {
  ...
  const isOpenapi31 = typeof openapi.openapi === 'string' && openapi.openapi.startsWith('3.1')
  ...
  switch (container) {
    ...
    case 'query':
      toOpenapiProp = function (propertyName, jsonSchemaElement, required) {
        ...
        const media = schemaToMediaRecursive(opts, jsonSchemaElement)
...
function resolveSchemaExamples (schema, opts) {
  if (opts.isOpenapi31) {
    // `examples` is a valid JSON Schema keyword, so it is left as it is
...
/**
 * In OpenAPI 3.1 the Schema Object is a JSON Schema, where `examples` is an
 * array of values and `example` is deprecated. Earlier versions require the
 * OpenAPI specific `example` / `examples` fields instead.
 */
function resolveSchemaExamplesRecursive (schema, opts) {
  resolveSchemaExamples(schema, opts)

  const getNestedSchemas = schemaTypeToNestedSchemas[schema.type]
  const nestedSchemas = getNestedSchemas?.(schema) ?? []

  for (const nestedSchema of nestedSchemas) {
    resolveSchemaExamplesRecursive(nestedSchema, opts)
  }
}

function schemaToMedia (opts, schema) {
  const media = { schema }
...
function schemaToMediaRecursive (opts, schema) {
  const media = schemaToMedia(opts, schema)
  resolveSchemaExamplesRecursive(schema, opts)
  return media
}
...
function prepareOpenapiSchemas (opts, jsonSchemas, ref) {
  ...
  for (const schemaName of Object.keys(jsonSchemas)) {
    ...
    resolveSchemaExamplesRecursive(openapiSchema, opts)
