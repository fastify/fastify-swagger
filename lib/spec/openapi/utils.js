...
function plainJsonObjectToOpenapi3 (opts, container, jsonSchema, externalSchemas, securityIgnores = [], openapiObject) {
  const localJsonSchema = resolveLocalRef(jsonSchema, externalSchemas)
  const obj = convertJsonSchemaToOpenapi3(opts, localJsonSchema, openapiObject)
  let toOpenapiProp
  switch (container) {
    case 'cookie':
    case 'header':
...
function resolveBodyParams (opts, body, schema, consumes, ref, openapiObject) {
  const resolved = convertJsonSchemaToOpenapi3(opts, ref.resolve(schema), openapiObject)
...
function resolveCommonParams (opts, container, parameters, schema, ref, sharedSchemas, securityIgnores, openapiObject) {
  const schemasPath = '#/components/schemas/'
  let resolved = convertJsonSchemaToOpenapi3(opts, ref.resolve(schema), openapiObject)
  ...
  const arr = plainJsonObjectToOpenapi3(opts, container, resolved, { ...sharedSchemas, ...ref.definitions().definitions }, securityIgnores, openapiObject)
...
function resolveResponse (opts, fastifyResponseJson, produces, ref, openapiObject) {
  ...
  statusCodes.forEach(statusCode => {
    ...
    const resolved = convertJsonSchemaToOpenapi3(opts, ref.resolve(rawJsonSchema), openapiObject)
...
function resolveCallbacks (opts, schema, ref, openapiObject) {
    ...
    for (const callbackUrl in eventSchema) {
      ...
      for (const httpMethodName in callbackSchema) {
        ...
        httpMethodContainer.responses = httpMethodSchema.responses
          ? convertJsonSchemaToOpenapi3(opts, ref.resolve(httpMethodSchema.responses), openapiObject)
...
function prepareOpenapiMethod (opts, schema, ref, openapiObject, url) {
  ...
  if (schema) {
    ...
    if (schema.querystring) resolveCommonParams(opts, 'query', parameters, schema.querystring, ref, openapiObject.definitions, securityIgnores.query, openapiObject)
    if (schema.body) {
      ...
      resolveBodyParams(opts, openapiMethod.requestBody, schema.body, schema.consumes, ref, openapiObject)
    }
    if (schema.params) resolveCommonParams(opts, 'path', parameters, schema.params, ref, openapiObject.definitions, undefined, openapiObject)
    if (schema.headers) resolveCommonParams(opts, 'header', parameters, schema.headers, ref, openapiObject.definitions, securityIgnores.header, openapiObject)
    // TODO: need to documentation, we treat it same as the querystring
    // fastify do not support cookies schema in first place
    if (schema.cookies) resolveCommonParams(opts, 'cookie', parameters, schema.cookies, ref, openapiObject.definitions, securityIgnores.cookie, openapiObject)
    ...
    if (schema.callbacks) openapiMethod.callbacks = resolveCallbacks(opts, schema.callbacks, ref, openapiObject)
  ...
  if ((!schema || !schema.params) && hasParams(url)) {
    ...
    resolveCommonParams(opts, 'path', parameters, schemaGenerated.params, ref, openapiObject.definitions, undefined, openapiObject)
    openapiMethod.parameters = parameters
  }

  openapiMethod.responses = resolveResponse(opts, schema ? schema.response : null, schema ? schema.produces : null, ref, openapiObject)

  return openapiMethod
}

function convertJsonSchemaToOpenapi3 (opts, jsonSchema, openapiObject) {
  if (typeof jsonSchema !== 'object' || jsonSchema === null) {
    return jsonSchema
  }

  if (Array.isArray(jsonSchema)) {
    return jsonSchema.map((s) => convertJsonSchemaToOpenapi3(opts, s, openapiObject))
  }
  ...
  for (const key of Object.keys(openapiSchema)) {
    ...
    if (key === 'definitions' || key === '$defs') {
      if (openapiObject?.components?.schemas && typeof value === 'object' && value !== null) {
        for (const defKey of Object.keys(value)) {
