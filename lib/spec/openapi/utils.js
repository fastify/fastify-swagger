...
function transformDefsToComponents (jsonSchema, inSchema = false) {
    ...
    Object.keys(jsonSchema).forEach(function (key) {
      if (key === 'properties') {
        Object.keys(jsonSchema[key]).forEach(function (prop) {
          jsonSchema[key][prop] = transformDefsToComponents(jsonSchema[key][prop], true)
        })
      } else if (key === '$ref') {
        jsonSchema[key] = jsonSchema[key].replace('definitions', 'components/schemas')
      } else if (key === 'examples') {
        if (!inSchema) {
          if (jsonSchema[key].length > 1) {
            jsonSchema.examples = convertExamplesArrayToObject(jsonSchema.examples)
          }
          // `else if` cause wrong coverage on this line, but the lines in block is covered, I don't know why
          if (jsonSchema[key].length === 1) {
        ...
        jsonSchema[key] = transformDefsToComponents(jsonSchema[key], inSchema)
      }
    })
  }
  return jsonSchema
