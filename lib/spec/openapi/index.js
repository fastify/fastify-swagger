...
module.exports = function (opts, cache, routes, Ref, done) {
  ...
  return function (opts) {
    ...
    const transformDocumentResult = defOpts.transformObject
      ? defOpts.transformObject({ openapiObject })
      : openapiObject

    if (opts && opts.yaml) {
      cache.string = yaml.stringify(transformDocumentResult, { strict: false })
      return cache.string
    }

    cache.object = transformDocumentResult
    return cache.object
  }
}
