...
module.exports = function (opts, routes, Ref, cache, done) {
  ...
  return function (opts) {
    ...
    for (const route of routes) {
      ...
      if (schema) {
        ...
        if (schema.body) {
          ...
          consumesAllFormOnly
            ? getParams('formData', parameters, schema.body)
      ...
      swaggerMethod.responses = generateResponse(schema ? schema.response : null)
    ...
    function getParams (container, parameters, form) {
      const resolved = ref.resolve(form)
      const add = plainJsonObjectToSwagger2(container, resolved, swaggerObject.definitions)
      add.forEach(_ => parameters.push(_))
    }

    // https://swagger.io/docs/specification/2-0/describing-responses/
    function generateResponse (fastifyResponseJson) {
